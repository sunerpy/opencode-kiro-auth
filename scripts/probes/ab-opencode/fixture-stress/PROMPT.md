...
6. Also maintain a CHECKSUM: the sum of every running total you have produced so far,
   taken modulo 9973. Recompute it from scratch after every hop by re-adding all
   running totals from hop 1 — never by incrementing a previous checksum.
7. Then follow `next`: read `ledger/<next>.json` on your NEXT turn.
8. When you read a file whose `next` is `null`, the chain is over. Stop reading and
   report the result.

The chain is 20 hops long. Every intermediate running total is an integer; if you
